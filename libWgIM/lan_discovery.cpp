...
static uint32_t send_broadcasts(Networking_Core* net, uint16_t port, const uint8_t* data, uint16_t length)
{
	...
	for (i = 0; i < broadcast_count; i++)
		net->sendpacket(broadcast_ip_port[i], data, length);
...
int send_LANdiscovery(uint16_t port, DHT * dht)
{
	...
	id_copy(data + 1, dht->selfPublicKey());

	send_broadcasts(dht->getNetwork().get(), port, data, 1 + crypto_box_PUBLICKEYBYTES);
	...
	/* IPv6 multicast */
	if (dht->getNetwork()->m_family == AF_INET6) {
		ip_port.ip = broadcast_ip(AF_INET6, AF_INET6);

		if (ip_isset(&ip_port.ip))
			if (dht->getNetwork()->sendpacket(ip_port, data, 1 + crypto_box_PUBLICKEYBYTES) > 0)
				res = 1;
	}

	/* IPv4 broadcast (has to be IPv4-in-IPv6 mapping if socket is AF_INET6 */
	ip_port.ip = broadcast_ip(dht->getNetwork()->m_family, AF_INET);

	if (ip_isset(&ip_port.ip))
		if (dht->getNetwork()->sendpacket( ip_port, data, 1 + crypto_box_PUBLICKEYBYTES))
			res = 1;
...
{
	dht->getNetwork()->networkingRegisterhandler(NET_PACKET_LAN_DISCOVERY, &handle_LANdiscovery, dht);
}

void LANdiscovery_kill(DHT * dht)
{
	dht->getNetwork()->networkingRegisterhandler( NET_PACKET_LAN_DISCOVERY, NULL, NULL);
}
