...
int NetWork::init(const IP& ip, uint16_t port_from, uint16_t port_to)
{
	if (port_from == 0 && port_to == 0)
	{
		port_from = TOX_PORTRANGE_FROM;
...
uint64_t NetWork::currentTimeActual(void)
{
	uint64_t time;
	/* This probably works fine */
	FILETIME ft;
 ...
 * return 0 on failure
 */
int NetWork::setSocketDualstack(sock_t sock)
{
	int ipv6only = 0;
	socklen_t optsize = sizeof(ipv6only);
	int res = getsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)& ipv6only, &optsize);
...
/* Basic network functions: Function to send packet(data) of length length to ip_port.
 *基本网络功能：将长度为length的数据包（数据）发送到ip_port的功能。*/
int NetWork::sendpacket(IP_Port ip_port, const uint8_t * data, uint16_t length)
{
	if (m_family == 0) /* Socket not initialized */
		return -1;
...
void NetWork::networkingRegisterhandler(uint8_t byte, packet_handler_callback cb, void* object)
{
