...
static void change_symmetric_key(Onion* onion)
{
	if (is_timeout(onion->timestamp, KEY_REFRESH_INTERVAL)) {
		CryptoCore::new_symmetric_key(onion->secret_symmetric_key);
 ...
 * return 0 on success.
 */
int create_onion_path(DHT * dht, Onion_Path * new_path, const Node_format * nodes)
{
	if (!new_path || !nodes)
		return -1;	
	CryptoCore::encryptPrecompute  (nodes[0].public_key, dht->selfSecretKey(), new_path->shared_key1);
	memcpy(new_path->public_key1, dht->selfPublicKey(), crypto_box_PUBLICKEYBYTES);
	...
	CryptoCore::encryptPrecompute(nodes[1].public_key, random_secret_key, new_path->shared_key2);
	memcpy(new_path->public_key2, random_public_key, crypto_box_PUBLICKEYBYTES);

	crypto_box_keypair(random_public_key, random_secret_key);
	CryptoCore::encryptPrecompute(nodes[2].public_key, random_secret_key, new_path->shared_key3);
 ...
 */
int create_onion_packet(uint8_t * packet, uint16_t max_packet_length, const Onion_Path * path, IP_Port dest,	const uint8_t * data, uint16_t length)
{
	if (1 + length + SEND_1 > max_packet_length || length == 0)
		return -1;

	uint8_t *step1=new uint8_t[SIZE_IPPORT + length];
	...
	uint8_t *step2=new uint8_t[SIZE_IPPORT + SEND_BASE + length];
	ipport_pack(step2, &path->ip_port3);
	memcpy(step2 + SIZE_IPPORT, path->public_key3, crypto_box_PUBLICKEYBYTES);

	int len =CryptoCore::encryptDataSymmetric (path->shared_key3, nonce, step1, sizeof(step1),step2 + SIZE_IPPORT + crypto_box_PUBLICKEYBYTES);

	if (len != SIZE_IPPORT + length + crypto_box_MACBYTES)
		return -1;

	uint8_t *step3=new uint8_t[SIZE_IPPORT + SEND_BASE * 2 + length];
	ipport_pack(step3, &path->ip_port2);
	memcpy(step3 + SIZE_IPPORT, path->public_key2, crypto_box_PUBLICKEYBYTES);
	len = CryptoCore::encryptDataSymmetric(path->shared_key2, nonce, step2, sizeof(step2),	step3 + SIZE_IPPORT + crypto_box_PUBLICKEYBYTES);
	...
	len = CryptoCore::encryptDataSymmetric(path->shared_key1, nonce, step3, sizeof(step3),	packet + 1 + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES);
	...
	const uint8_t * data, uint16_t length)
{
	...
	len = CryptoCore::encryptDataSymmetric(path->shared_key2, nonce, step2, sizeof(step2),	packet + crypto_box_NONCEBYTES + SIZE_IPPORT + crypto_box_PUBLICKEYBYTES);
...
int send_onion_packet(Networking_Core * net, const Onion_Path * path, IP_Port dest, const uint8_t * data, uint16_t length)
{
	...
	if ( net->sendpacket(path->ip_port1, packet, len) != len)
		return -1;
...
int send_onion_response(Networking_Core * net, IP_Port dest, const uint8_t * data, uint16_t length, const uint8_t * ret)
{
	...
	uint8_t *packet=new uint8_t[1 + RETURN_3 + length];
	packet[0] = NET_PACKET_ONION_RECV_3;
	memcpy(packet + 1, ret, RETURN_3);
	memcpy(packet + 1 + RETURN_3, data, length);

	if ((uint32_t)net->sendpacket(dest, packet, sizeof(packet)) != sizeof(packet))
		return -1;
...
static int handle_send_initial(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	get_shared_key(&onion->shared_keys_1, shared_key, onion->dht->selfSecretKey(), packet + 1 + crypto_box_NONCEBYTES);
	int len =CryptoCore::decryptDataSymmetric(shared_key, packet + 1, packet + 1 + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES,
...
int onion_send_1(const Onion * onion, const uint8_t * plain, uint16_t len, IP_Port source, const uint8_t * nonce)
{
	...
	CryptoCore::newNonce(ret_part);
	len = CryptoCore::encryptDataSymmetric(onion->secret_symmetric_key, ret_part, ip_port, SIZE_IPPORT,
	...
	if ((uint32_t)onion->net->sendpacket(send_to, data, data_len) != data_len)
		return 1;
...
static int handle_send_1(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	uint8_t plain[ONION_MAX_PACKET_SIZE];
	uint8_t shared_key[crypto_box_BEFORENMBYTES];
	get_shared_key(&onion->shared_keys_2, shared_key, onion->dht->selfSecretKey(), packet + 1 + crypto_box_NONCEBYTES);
	int len = CryptoCore::decryptDataSymmetric(shared_key, packet + 1, packet + 1 + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES,
	...
	len =CryptoCore::encryptDataSymmetric(onion->secret_symmetric_key, ret_part, ret_data, sizeof(ret_data),
		ret_part + crypto_box_NONCEBYTES);
...
static int handle_send_2(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	uint8_t plain[ONION_MAX_PACKET_SIZE];
	uint8_t shared_key[crypto_box_BEFORENMBYTES];
	get_shared_key(&onion->shared_keys_3, shared_key, onion->dht->selfSecretKey(), packet + 1 + crypto_box_NONCEBYTES);
	int len =CryptoCore::decryptDataSymmetric(shared_key, packet + 1, packet + 1 + crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES,
	...
	len =CryptoCore::encryptDataSymmetric(onion->secret_symmetric_key, ret_part, ret_data, sizeof(ret_data),ret_part + crypto_box_NONCEBYTES);
...
static int handle_recv_3(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	int len =CryptoCore::decryptDataSymmetric(onion->secret_symmetric_key, packet + 1, packet + 1 + crypto_box_NONCEBYTES,SIZE_IPPORT + RETURN_2 + crypto_box_MACBYTES, plain);
...
static int handle_recv_2(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	int len =CryptoCore::decryptDataSymmetric(onion->secret_symmetric_key, packet + 1, packet + 1 + crypto_box_NONCEBYTES,
...
static int handle_recv_1(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	if ((uint32_t)onion->net->sendpacket(send_to, packet + (1 + RETURN_1), data_len) != data_len)
		return 1;
...
Onion* new_onion(DHT * dht)
{
	...
	onion->dht = dht;
	onion->net = dht->getNetwork().get();
	 CryptoCore::new_symmetric_key(onion->secret_symmetric_key);
	onion->timestamp = unix_time();

	onion->net->networkingRegisterhandler( NET_PACKET_ONION_SEND_INITIAL, &handle_send_initial, onion);
	onion->net->networkingRegisterhandler( NET_PACKET_ONION_SEND_1, &handle_send_1, onion);
	onion->net->networkingRegisterhandler(NET_PACKET_ONION_SEND_2, &handle_send_2, onion);

	onion->net->networkingRegisterhandler(NET_PACKET_ONION_RECV_3, &handle_recv_3, onion);
	onion->net->networkingRegisterhandler(NET_PACKET_ONION_RECV_2, &handle_recv_2, onion);
	onion->net->networkingRegisterhandler(NET_PACKET_ONION_RECV_1, &handle_recv_1, onion);
...
void kill_onion(Onion * onion)
{
	...
	onion->net->networkingRegisterhandler( NET_PACKET_ONION_SEND_INITIAL, NULL, NULL);
	onion->net->networkingRegisterhandler( NET_PACKET_ONION_SEND_1, NULL, NULL);
	onion->net->networkingRegisterhandler( NET_PACKET_ONION_SEND_2, NULL, NULL);

	onion->net->networkingRegisterhandler( NET_PACKET_ONION_RECV_3, NULL, NULL);
	onion->net->networkingRegisterhandler(NET_PACKET_ONION_RECV_2, NULL, NULL);
	onion->net->networkingRegisterhandler( NET_PACKET_ONION_RECV_1, NULL, NULL);
