...
int onion_add_bs_path_node(Onion_Client* onion_c, IP_Port ip_port, const uint8_t* public_key)
{
	...
	for (i = 0; i < MAX_PATH_NODES; ++i) {
		if (CryptoCore::publicKeyCmp  (public_key, onion_c->path_nodes_bs[i].public_key) == 0)
			return -1;
	}
...
static int onion_add_path_node(Onion_Client * onion_c, IP_Port ip_port, const uint8_t * public_key)
{
	...
	for (i = 0; i < MAX_PATH_NODES; ++i) {
		if (CryptoCore::publicKeyCmp(public_key, onion_c->path_nodes[i].public_key) == 0)
			return -1;
	}
	...
	const uint8_t * data, uint16_t length)
{
	if (path->ip_port1.ip.family == AF_INET || path->ip_port1.ip.family == AF_INET6) {
		...
		if (onion_c->net->sendpacket( path->ip_port1, packet, len) != len)
			return -1;
	...
	uint8_t is_stored, const uint8_t * pingid_or_key, uint32_t path_num)
{
	...
	if (num == 0) {
		...
		if (is_stored == 1 && CryptoCore::publicKeyCmp(pingid_or_key, onion_c->temp_public_key) != 0) {
			is_stored = 0;
		}
	...
	for (i = 0; i < list_length; ++i) {
		if (CryptoCore::publicKeyCmp(list_nodes[i].public_key, public_key) == 0) {
			index = i;
			stored = 1;
			break;
		}
...
static int good_to_ping(Last_Pinged * last_pinged, uint8_t * last_pinged_index, const uint8_t * public_key)
{
	...
	for (i = 0; i < MAX_STORED_PINGED_NODES; ++i) {
		if (!is_timeout(last_pinged[i].timestamp, MIN_NODE_PING_TIME))
			if (CryptoCore::publicKeyCmp(last_pinged[i].public_key, public_key) == 0)
				return 0;
	}
	...
	for (i = 0; i < num_nodes; ++i) {
		...
		if (is_timeout(list_nodes[0].timestamp, ONION_NODE_TIMEOUT)
			...
			for (j = 0; j < list_length; ++j) {
				if (CryptoCore::publicKeyCmp(list_nodes[j].public_key, nodes[i].public_key) == 0) {
					break;
				}
			}
...
static int handle_announce_response(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	uint8_t* plain=new uint8_t[1 + ONION_PING_ID_SIZE + len_nodes];
	int len = -1;

	if (num == 0) {
		len = CryptoCore:: CryptoCore::decrypt_data (public_key, onion_c->c->self_secret_key, packet + 1 + ONION_ANNOUNCE_SENDBACK_DATA_LENGTH,
		...
		len =  CryptoCore:: CryptoCore::decrypt_data(public_key, onion_c->friends_list[num - 1].temp_secret_key,
...
static int handle_data_response(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	uint8_t *temp_plain=new uint8_t[length - ONION_DATA_RESPONSE_MIN_SIZE];
	int len =  CryptoCore:: CryptoCore::decrypt_data(packet + 1 + crypto_box_NONCEBYTES, onion_c->temp_secret_key, packet + 1,
	...
	uint8_t *plain=new uint8_t[length - ONION_DATA_RESPONSE_MIN_SIZE - DATA_IN_RESPONSE_MIN_SIZE];
	len =  CryptoCore:: CryptoCore::decrypt_data(temp_plain, onion_c->c->self_secret_key, packet + 1, temp_plain + crypto_box_PUBLICKEYBYTES,
...
static int handle_dhtpk_announce(void* object, const uint8_t * source_pubkey, const uint8_t * data, uint16_t length)
{
	...
	if (len_nodes != 0) {
		...
		for (i = 0; i < num_nodes; ++i) {
			...
			{
				onion_c->dht->getnodes( nodes[i].ip_port, nodes[i].public_key, onion_c->friends_list[friend_num].dht_public_key,NULL);				
			}
...
int send_onion_data(Onion_Client * onion_c, int friend_num, const uint8_t * data, uint16_t length)
{
	...
	uint8_t *packet=new uint8_t[DATA_IN_RESPONSE_MIN_SIZE + length];
	memcpy(packet, onion_c->c->self_public_key, crypto_box_PUBLICKEYBYTES);
	int len =CryptoCore::encrypt_data(onion_c->friends_list[friend_num].real_public_key, onion_c->c->self_secret_key, nonce, data,
...
static int send_dht_dhtpk(const Onion_Client * onion_c, int friend_num, const uint8_t * data, uint16_t length)
{
	...
	uint8_t *temp=new uint8_t[DATA_IN_RESPONSE_MIN_SIZE + crypto_box_NONCEBYTES + length];
	...
	len =CryptoCore::create_request(onion_c->dht->m_selfPublicKey, onion_c->dht->m_selfSecretKey, packet,
		onion_c->friends_list[friend_num].dht_public_key, temp, sizeof(temp), CRYPTO_PACKET_DHTPK);

	if (len == -1)
		return -1;

	return onion_c->dht->routeone_tofriend ( onion_c->friends_list[friend_num].dht_public_key, packet, len);
}
	...
	uint16_t length)
{
	...
	int len =  CryptoCore::decrypt_data(packet, onion_c->c->self_secret_key, packet + crypto_box_PUBLICKEYBYTES,
	...
	if (CryptoCore::publicKeyCmp(source_pubkey, plain + 1 + sizeof(uint64_t)) != 0)
		return 1;
...
static int send_dhtpk_announce(Onion_Client * onion_c, uint16_t friend_num, uint8_t onion_dht_both)
{
	...
	memcpy(data + 1 + sizeof(uint64_t), onion_c->dht->m_selfPublicKey, crypto_box_PUBLICKEYBYTES);
	Node_format nodes[MAX_SENT_NODES];
...
int onion_friend_num(const Onion_Client * onion_c, const uint8_t * public_key)
{
	...
	for (i = 0; i < onion_c->num_friends; ++i) {
		...
		if (CryptoCore::publicKeyCmp(public_key, onion_c->friends_list[i].real_public_key) == 0)
			return i;
	}
...
int onion_set_friend_DHT_pubkey(Onion_Client * onion_c, int friend_num, const uint8_t * dht_key)
{
	...
	if (onion_c->friends_list[friend_num].know_dht_public_key) {
		if (CryptoCore::publicKeyCmp(dht_key, onion_c->friends_list[friend_num].dht_public_key) == 0) {
			return -1;
		}
...
int onion_getfriendip(const Onion_Client * onion_c, int friend_num, IP_Port * ip_port)
{
	...
	return  onion_c->dht->getfriendip(dht_public_key, ip_port);
}
...
Onion_Client* new_onion_client(Net_Crypto * c)
{
	...
	onion_c->dht = c->m_dht.get();
	onion_c->net = c->m_dht->m_net.get();
	onion_c->c = c;
	CryptoCore::CryptoCore::new_symmetric_key (onion_c->secret_symmetric_key);
	crypto_box_keypair(onion_c->temp_public_key, onion_c->temp_secret_key);
	onion_c->net->networkingRegisterhandler ( NET_PACKET_ANNOUNCE_RESPONSE, &handle_announce_response, onion_c);
	onion_c->net->networkingRegisterhandler(NET_PACKET_ONION_DATA_RESPONSE, &handle_data_response, onion_c);
...
void kill_onion_client(Onion_Client * onion_c)
{
	...
	onion_c->net->networkingRegisterhandler( NET_PACKET_ANNOUNCE_RESPONSE, NULL, NULL);
	onion_c->net->networkingRegisterhandler( NET_PACKET_ONION_DATA_RESPONSE, NULL, NULL);
