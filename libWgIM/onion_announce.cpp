	...
	const uint8_t* data_public_key, uint64_t sendback_data)
{
	...
	int len =CryptoCore::encrypt_data(dest_client_id, secret_key, packet + 1, plain, sizeof(plain),
	...
	const uint8_t * encrypt_public_key, const uint8_t * nonce, const uint8_t * data, uint16_t length)
{
	...
	int len = CryptoCore::encrypt_data(encrypt_public_key, random_secret_key, packet + 1 + crypto_box_PUBLICKEYBYTES, data, length,
...
static int in_entries(const Onion_Announce * onion_a, const uint8_t * public_key)
{
	...
	for (i = 0; i < ONION_ANNOUNCE_MAX_ENTRIES; ++i) {
		if (!is_timeout(onion_a->entries[i].time, ONION_ANNOUNCE_TIMEOUT)
			&&CryptoCore::publicKeyCmp(onion_a->entries[i].public_key, public_key) == 0)
			return i;
	}
	...
	const uint8_t * data_public_key, const uint8_t * ret)
{
	...
	if (pos == -1) {
		if (id_closest(onion_a->dht->selfPublicKey(), public_key, onion_a->entries[0].public_key) == 1)
			pos = 0;
	}
	...
	memcpy(cmp_public_key, onion_a->dht->selfPublicKey(), crypto_box_PUBLICKEYBYTES);
...
static int handle_announce_request(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	get_shared_key(&onion_a->shared_keys_recv, shared_key, onion_a->dht->selfSecretKey(), packet_public_key);
	...
	unsigned int num_nodes = onion_a->dht->get_close_nodes( plain + ONION_PING_ID_SIZE, nodes_list, 0,LAN_ip(source.ip) == 0, 1);
	...
	if (index == -1) {
		...
		if (CryptoCore::publicKeyCmp(onion_a->entries[index].public_key, packet_public_key) == 0) {
			if (CryptoCore::publicKeyCmp(onion_a->entries[index].data_public_key, data_public_key) != 0) {
				pl[0] = 0;
				memcpy(pl + 1, ping_id2, ONION_PING_ID_SIZE);
			}
			else {
	...
	len = CryptoCore::encryptDataSymmetric(shared_key, nonce, pl, 1 + ONION_PING_ID_SIZE + nodes_length,
...
static int handle_data_request(void* object, IP_Port source, const uint8_t * packet, uint16_t length)
{
	...
	uint8_t *data=new uint8_t[length - (crypto_box_PUBLICKEYBYTES + ONION_RETURN_3)];
...
Onion_Announce * new_onion_announce(DHT * dht)
{
	...
	onion_a->dht = dht;
	onion_a->net = dht->getNetwork().get();
	 CryptoCore::new_symmetric_key(onion_a->secret_bytes);

	 onion_a->net->networkingRegisterhandler(NET_PACKET_ANNOUNCE_REQUEST, &handle_announce_request, onion_a);
	 onion_a->net->networkingRegisterhandler(NET_PACKET_ONION_DATA_REQUEST, &handle_data_request, onion_a);
...
void kill_onion_announce(Onion_Announce * onion_a)
{
	...
	onion_a->net->networkingRegisterhandler(  NET_PACKET_ANNOUNCE_REQUEST, NULL, NULL);
	onion_a->net->networkingRegisterhandler(  NET_PACKET_ONION_DATA_REQUEST, NULL, NULL);
	free(onion_a);
}
